...
const getCompanyInitials = (name: string): string => {
  ...
  const initials = name
    ...
    .join('')
    .toUpperCase();
  return initials ? `W-${initials}` : 'WHS';
};
...
export const createOrder = async (req: AuthRequest, res: Response) => {
  ...
  try {
    ...
    const initials = getCompanyInitials(setting?.companyName || 'WHS');
    ...
    const order = await WholesaleOrder.create({
      ...
      approvedBy: (isAdminPayment || paymentStatus === 'paid') ? (approved_by || (req.user ? (req.user.name || req.user.email) : 'Admin')) : null,
...
export const getOrders = async (req: AuthRequest, res: Response) => {
  try {
    ...
    const total = await WholesaleOrder.countDocuments(filter);
...
export const getOrderById = async (req: Request, res: Response) => {
  try {
    const order = await WholesaleOrder.findById(req.params.id).populate('userId', 'name email');
    if (!order) {
  ...
  try {
    ...
    if (finalPaymentStatus === 'paid') {
      if (!order.receiptNumber) {
        ...
        const count = await WholesaleOrder.countDocuments({ receiptNumber: { $regex: new RegExp(`^${prefix}-`) } });
        updateData.receiptNumber = `${prefix}-${count + 1}`;
      }
      updateData.approvedBy = req.user?.name || req.user?.email || 'Admin';
    }

    const updatedOrder = await WholesaleOrder.findByIdAndUpdate(
      req.params.id,
      { $set: updateData },
      { new: true }
    );
...
export const bulkUpdateStatus = async (req: AuthRequest, res: Response) => {
  ...
  try {
    ...
    for (const id of ids) {
      ...
      const updated = await WholesaleOrder.findByIdAndUpdate(id, { $set: updateData }, { new: true });
...
export const bulkDeleteOrders = async (req: Request, res: Response) => {
  ...
  try {
    if (req.query.permanent === 'true') {
      await WholesaleOrder.deleteMany({ _id: { $in: ids } });
    } else {
      await WholesaleOrder.updateMany(
        { _id: { $in: ids } },
...
export const bulkRestoreOrders = async (req: Request, res: Response) => {
  ...
  try {
    ...
    res.json({ message: 'Wholesale orders restored successfully' });
  } catch (error: any) {
    res.status(500).json({ message: error.message });
  }
};
